    ...
    async function fetchEscultor(slug) {
        ...
        try {
            const response = await axios.get(
                `${import.meta.env.VITE_API_URL}/api/escultores/${slug}`,
                {
                    params: { nombre: slug },
                },
            );
...
{#if !mostrandoCarga}
    <article
        class="escultor-detail-container max-w-2xl mx-auto mt-8 p-6 bg-white shadow-lg rounded-lg animate"
    >
        <header class="text-center">
            <img
                src={escultor.escultorFoto}
                alt={escultor.escultorName}
                class="w-32 h-32 rounded-full mx-auto"
            />
            <h1 class="text-3xl font-bold mt-4">
                {escultor.escultorName}
                <span class={`fi fi-${escultor.nacionalidad}`}></span>
            </h1>
        </header>
        ...
        <section class="mt-4">
            ...
            <p>
                Email: <a
                    href="mailto:{escultor.contactoEmail}"
                    class="text-blue-600 hover:underline"
                    >{escultor.contactoEmail}</a
                >
            </p>
        </section>
    </article>
